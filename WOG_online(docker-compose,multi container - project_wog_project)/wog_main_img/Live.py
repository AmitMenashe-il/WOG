...
def outcome():
    ...
    if not session['outcome']:
        ...
        return render_template('endgame.html', message=f"you lost! you have {session['score']} points.")
    else:
        del session['outcome']
        added_score = 5 + 3 * int(session['difficulty'])
        session['score'] += added_score
        add_score(session['name'],session['score'])
        del session['difficulty']
        return render_template('endgame.html', message=f"you won! you win {added_score} points and now have a total of {session['score']} points!")
